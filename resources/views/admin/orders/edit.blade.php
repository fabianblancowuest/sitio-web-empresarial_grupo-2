...
@section('title', 'Editar Pedido · Admin CodeBridge')
...
<div class="mb-8">
    ...
    <h1 class="font-display text-xl sm:text-2xl font-bold text-ink dark:text-white">Editar pedido</h1>
    <p class="text-slate-500 dark:text-slate-400 text-sm mt-1">{{ $order->title }}</p>
</div>
...
<div class="max-w-2xl">
    <form action="{{ route('admin.orders.update', $order) }}" method="POST" class="space-y-5">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-5 sm:p-6 space-y-5">
            <h2 class="font-display font-bold text-xs uppercase tracking-wide text-slate-400">Datos del proyecto</h2>
            ...
            <div>
                ...
                <select name="user_id" class="w-full border border-slate-200 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500">
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                          class="w-full border border-slate-200 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 resize-none">{{ old('description', $order->description) }}</textarea>
            </div>
        </div>

        <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-5 sm:p-6 space-y-5">
            <h2 class="font-display font-bold text-xs uppercase tracking-wide text-slate-400">Estado y condiciones</h2>
            ...
            <div class="grid sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="date" name="deadline" value="{{ old('deadline', $order->deadline ? \Carbon\Carbon::parse($order->deadline)->format('Y-m-d') : '') }}"
                           class="w-full border border-slate-200 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500">
                </div>
            </div>
        </div>
        ...
        <div class="flex gap-3">
            ...
            <a href="{{ route('admin.orders.index') }}" class="border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 font-semibold px-5 py-2.5 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors text-sm">
                Cancelar
            </a>
        </div>
    </form>
