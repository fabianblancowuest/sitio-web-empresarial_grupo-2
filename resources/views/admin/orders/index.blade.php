...
<div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-8">
    <div>
        <h1 class="font-display text-xl sm:text-2xl font-bold text-ink dark:text-white">Pedidos</h1>
        <p class="text-slate-500 dark:text-slate-400 text-sm mt-1">{{ $orders->count() }} proyecto{{ $orders->count() !== 1 ? 's' : '' }} en total</p>
    </div>
    <a href="{{ route('admin.orders.create') }}"
       class="inline-flex items-center gap-2 bg-brand-500 hover:bg-brand-600 text-white font-semibold px-4 py-2.5 rounded-lg transition-colors text-sm shrink-0">
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-slate-100 dark:border-slate-700">
                    <th class="text-left px-5 py-3.5 text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Cliente</th>
                    <th class="text-left px-5 py-3.5 text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Proyecto</th>
                    <th class="text-left px-5 py-3.5 text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Estado</th>
                    <th class="text-left px-5 py-3.5 text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider hidden md:table-cell">Precio</th>
                    <th class="text-left px-5 py-3.5 text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider hidden lg:table-cell">Entrega</th>
                ...
                <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/40 transition-colors">
                    <td class="px-5 py-4">
                        <div class="flex items-center gap-2.5">
                            ...
                            <span class="font-medium text-ink dark:text-white text-sm">{{ $order->user->name ?? '—' }}</span>
                        </div>
                    </td>
                    <td class="px-5 py-4">
                        <p class="font-medium text-slate-700 dark:text-slate-200 text-sm">{{ $order->title }}</p>
                    ...
                    <td class="px-5 py-4 text-slate-500 dark:text-slate-400 font-medium hidden md:table-cell">
                        {{ $order->price ? '$' . number_format($order->price, 2) : '—' }}
                    </td>
                    <td class="px-5 py-4 text-slate-500 dark:text-slate-400 hidden lg:table-cell">
                        {{ $order->deadline ? \Carbon\Carbon::parse($order->deadline)->format('d/m/Y') : '—' }}
                    </td>
                    <td class="px-5 py-4">
                        <div class="flex items-center justify-end gap-2">
                            <a href="{{ route('admin.orders.show', $order) }}"
                               class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 rounded-lg transition-colors">
                            ...
                            <a href="{{ route('admin.orders.edit', $order) }}"
                               class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-brand-600 dark:text-brand-400 bg-brand-50 dark:bg-brand-900/20 hover:bg-brand-100 dark:hover:bg-brand-900/40 rounded-lg transition-colors">
                ...
                <tr>
                    <td colspan="6" class="px-5 py-16 text-center">
                        <div class="flex flex-col items-center gap-3 text-slate-400">
                            ...
                            <p class="font-display font-bold text-slate-500 dark:text-slate-400">Sin pedidos</p>
