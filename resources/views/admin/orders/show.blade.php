...
<div class="mb-8">
    <a href="{{ route('admin.orders.index') }}"
       class="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-brand-500 transition-colors mb-4">
    ...
    <h1 class="font-display text-2xl font-extrabold text-ink dark:text-white">{{ $order->title }}</h1>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-1 space-y-6">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6">
            <h2 class="font-display font-bold text-sm uppercase tracking-wide text-slate-400 mb-4">Detalles</h2>
        ...
        {{-- Cambiar estado --}}
        <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6">
            <h2 class="font-display font-bold text-sm uppercase tracking-wide text-slate-400 mb-4">Cambiar estado</h2>
            <form action="{{ route('admin.orders.status', $order) }}" method="POST" class="space-y-4">
                ...
                <button type="submit" class="w-full bg-brand-500 hover:bg-brand-600 text-white font-semibold px-4 py-2.5 rounded-xl transition-colors text-sm">
                    Actualizar estado
                </button>
            </form>
        </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6">
            <h2 class="font-display font-bold text-sm uppercase tracking-wide text-slate-400 mb-3">Descripción del proyecto</h2>
    ...
    <div class="lg:col-span-2">
        <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden flex flex-col h-[600px]">
            ...
            <div class="flex-1 overflow-y-auto p-6 space-y-4" id="messages-container">
                ...
                <div class="flex {{ $isAdmin ? 'justify-end' : 'justify-start' }}">
                    <div class="max-w-[80%] {{ $isAdmin ? 'bg-brand-500 text-white rounded-2xl rounded-br-md' : 'bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-2xl rounded-bl-md' }} px-4 py-3">
