...
<div class="mb-8">
    ...
    <h1 class="font-display text-3xl font-extrabold text-ink dark:text-white mt-2">Agregar Desarrollador</h1>
</div>
...
<div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 p-8 max-w-2xl">
    <form action="{{ route('admin.developers.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
        ...
        <div>
            ...
            <input type="email" name="email" value="{{ old('email') }}" placeholder="Ej: fabian@teamtwo.com"
                   class="w-full border border-slate-200 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500">
        </div>
        ...
        <div>
            ...
            <textarea name="bio" rows="4" placeholder="Descripción del developer..."
                      class="w-full border border-slate-200 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 resize-none">{{ old('bio') }}</textarea>
        </div>
