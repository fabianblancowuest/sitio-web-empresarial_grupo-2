...
<div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-8">
    <div>
        <h1 class="font-display text-xl sm:text-2xl font-bold text-ink dark:text-white">Desarrolladores</h1>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-slate-100 dark:border-slate-700">
                    <th class="text-left px-5 py-3.5 text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Desarrollador</th>
                    <th class="text-left px-5 py-3.5 text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider hidden sm:table-cell">Rol</th>
                    <th class="text-left px-5 py-3.5 text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider hidden md:table-cell">Email</th>
                    <th class="text-left px-5 py-3.5 text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider hidden lg:table-cell">Habilidades</th>
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                ...
                <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/40 transition-colors">
                        ...
                        <div class="flex items-center gap-3">
                            @if($developer->photo)
                                <img src="{{ asset($developer->photo) }}"
                                     alt="{{ $developer->name }}"
                                     class="w-9 h-9 rounded-full object-cover shrink-0 border border-slate-200 dark:border-slate-600"
                                     style="object-position: center top;">
                            @else
                                <div class="w-9 h-9 rounded-full {{ $color }} flex items-center justify-center text-xs font-bold font-display shrink-0">
                                    {{ $initials }}
                                </div>
                            @endif
                            <div>
                                <p class="font-semibold text-ink dark:text-white leading-none text-sm">{{ $developer->name }}</p>
                                @if($developer->user)
                    ...
                    <td class="px-5 py-4 text-slate-500 dark:text-slate-400 hidden sm:table-cell">{{ $developer->role ?? '—' }}</td>
                    <td class="px-5 py-4 text-slate-500 dark:text-slate-400 hidden md:table-cell text-xs">{{ $developer->email ?? '—' }}</td>
                ...
                <tr>
                    <td colspan="5" class="px-5 py-16 text-center">
                        <div class="flex flex-col items-center gap-3 text-slate-400">
                            ...
                            <p class="font-display font-bold text-slate-500 dark:text-slate-400">Sin desarrolladores</p>
