        ...
        <div>
            ...
            <div class="flex items-center gap-4">
                <div class="w-14 h-14 rounded-full overflow-hidden bg-brand-100 dark:bg-brand-900/40 flex items-center justify-center shrink-0">
                    @if ($user->photo)
                        <img src="{{ asset($user->photo) }}" alt="Foto" class="w-full h-full object-cover">
                    @else
                        <span class="text-lg font-bold font-display text-brand-700 dark:text-brand-400">{{ strtoupper(substr($user->name, 0, 1)) }}</span>
                    @endif
                </div>
                <div class="flex-1">
                    <label class="cursor-pointer inline-flex items-center gap-2 bg-brand-50 dark:bg-brand-900/20 text-brand-700 dark:text-brand-400 hover:bg-brand-100 dark:hover:bg-brand-900/40 font-semibold px-4 py-2 rounded-lg transition-colors text-sm">
